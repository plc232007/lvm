...
function marcaAnguloReto(t: TrianguloRetangulo): [number, number][] {
  ...
  return [t.A, somar(t.A, paraB), somar(somar(t.A, paraB), paraC), somar(t.A, paraC)].map(par);
}
...
function rotuloLado(p: Ponto, q: Ponto, oposto: Ponto, distancia = 0.5): [number, number] {
  ...
  const paraFora = produtoEscalar(subtrair(centroLado, oposto), normal) >= 0 ? 1 : -1;
...
export function FiguraTriangulo({
  ...
  return (
    <Mafs
      ...
      ssr
    >
      <Polygon points={[par(t.A), par(t.B), par(t.C)]} color={Theme.blue} fillOpacity={0.12} />
      <Polygon points={marcaAnguloReto(t)} color={Theme.foreground} fillOpacity={0} weight={2} />

      <Line.Segment point1={par(t.A)} point2={par(t.H)} color={Theme.green} style="dashed" />
      ...
      <RotuloMafs at={[t.B.x + (t.H.x - t.B.x) / 2, yProjecoes - 0.5]} tex="n" color={Theme.orange} />
      <RotuloMafs at={[t.H.x + (t.C.x - t.H.x) / 2, yProjecoes - 0.5]} tex="m" color={Theme.pink} />
